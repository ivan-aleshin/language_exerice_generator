...
class ExerciseGenerator:
    ...
    _line_error_text = """Translation error caused confusion, due to
    inadvertent linguistic substitution, during the automated
    process.""".replace('\n', '')

    _block_error_text = """We regret to inform you that an unforeseen error
    has occurred within our app, resulting in an inconvenience for our valued
    users. Our dedicated team is diligently investigating the issue and working
    tirelessly to rectify it promptly. We sincerely apologize for any
    disruption caused and kindly request your patience as we strive to provide
    a seamless and enhanced user experience. Thank you for your understanding
    ...
    def text_to_dataset(text: str):
        """
        Returns dataset with text splitted into paragraphs (blocks) in first
        column and length of each block in second one.
        """
        text = ExerciseGenerator.text_cleaning(text)
        blocks = [block.strip() for block in text.split('\n') if block.strip()]
        block_lengths = list(
            map(lambda x: len(ExerciseGenerator.split_blocks(x)), blocks)
            )
        text_data = pd.DataFrame(zip(blocks, block_lengths),
                                 columns=['block', 'block_length'])
        text_data = ExerciseGenerator.explode_dataset(text_data)
        text_data['part_of_word'] = text_data['line'].apply(
            ExerciseGenerator.add_part_of_word)
    ...
    def explode_dataset(dataset):
        ...
        of sentences.
        """
        dataset['line'] = dataset['block'].apply(
            ExerciseGenerator.split_blocks)
        dataset = dataset.explode('line').reset_index()
        dataset['line_length'] = dataset['line'].apply(
    ...
    def choose_sentence(self, option=None, limits=(3, 15)):
        ...
        index_tag = self.text_data[
            (self.text_data['part_of_word'].str.contains(option))].index
        index_limit = self.text_data[
            (self.text_data['line_length'] >= limits[0]) &
    ...
    def choose_block(self, option=None, limits=(40, 150)):
        ...
        index_tag = self.text_data[
            (self.text_data['part_of_word'].str.contains(option))].index
        index_limit = self.text_data[
            (self.text_data['block_words'] >= limits[0]) &
            (self.text_data['block_words'] <= limits[1])].index
        if index_limit.empty:
            max_len = self.text_data['block_words'].max()
            index_limit = self.text_data.loc[
                self.text_data['block_words'] == max_len].index
            del max_len
        final_index = pd.Index.intersection(index_tag, index_limit)
    ...
    def gen_missings(self, options=True):
        ...
        if options:
            answers = [target_word]
            wrong_answers = map(
                lambda x: x[0], self.word_vectors.most_similar(
                    target_word.lower())[:3])
    ...
    def gen_aud_text(self):
        ...
        target_words = [
            str(token).lower() for token in ExerciseGenerator._nlp(sentence)
    ...
    def gen_part_of_word(self):
        ...
        sentence = [
            (str(token).lower(), str(token.pos_))
            if (str(token) in target_words and str(token.pos_ in tags))
            else (str(token) + ' ')
            for token in ExerciseGenerator._nlp(sentence)
            ]
        return ('part_of_word',
                [
    ...
    def gen_sentence_order(self):
        ...
        wrong_sents = [' '.join(
            sentence[: shuffle_range] + list(slice) + sentence[shuffle_range + 4:]
    ...
    def target_word_selector(sentence, tag):
        target_words = [
            str(token) for token in ExerciseGenerator._nlp(sentence)
            if token.pos_ == tag]
        if target_words:
        ...
        return random.choice(
            [str(token) for token in ExerciseGenerator._nlp(sentence)
             if str(token.pos_) in ExerciseGenerator._all_tags])
